<?php
  ...
  if (isset($_POST['id_person']) && isset($_POST['title']) || !isset($_POST['id']) || !is_numeric($_POST['id'])){
  
    if (!isset($_POST['keywords']) || !is_array($_POST['keywords']) || sizeof($_POST['keywords']) < 4){
      $referer = "index.php?pid=223&id=" .$_POST['id'];
      $_SESSION['success'] = 0;
    ...
    try{
      $redaction = new Miki_redaction($_POST['id']);
      ...
      try{
        ...
        $_SESSION['success'] = 0;
        $_SESSION['msg'] = "Une erreur est survenue lors de la modification de la rédaction : <br />" .$e->getMessage();
        echo "<script type='text/javascript'>document.location='$referer';</script>";
        exit();
      }
    }
  ...
  // ajoute le message à la session
  $_SESSION['success'] = 1;
  $_SESSION['msg'] = _("La rédaction a été modifiée avec succès");
  // puis redirige vers la page précédente
