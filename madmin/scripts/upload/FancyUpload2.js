...
(function() {
	var phrases = {
		...
		'validationErrors': {
			...
			'sizeLimitMin': 'Le fichier <em>{name}</em> (<em>{size}</em>) est trop petit, la taille minimale est {fileSizeMin}.',
			'sizeLimitMax': 'Le fichier <em>{name}</em> (<em>{size}</em>) est trop grand, la taille maximale est de <em>{fileSizeMax}</em>.',
			'fileListMax': 'Le fichier <em>{name}</em> n\'a pas pu être ajouté, le nombre maximum de <em>{fileListMax} fichiers</em> a été dépassé.',
			'fileListSizeMax': 'Le fichier <em>{name}</em> (<em>{size}</em>) est trop grand, la somme totale de poid des fichiers de <em>{fileListSizeMax}</em> a été dépassée.'
		},
		'errors': {
