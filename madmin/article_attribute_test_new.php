<?php
  ...
  $sql = sprintf("INSERT INTO miki_shop_article_attribute (name, code, type, value) VALUES('%s', '%s', %d, '%s')",
  ...
  if (!$result){
    ...
    // ajoute le message à la session
    $_SESSION['success'] = 0;
    $_SESSION['msg'] = _("Erreur lors de l'insertion de l'attribut dans la base de données : ") ."<br />$sql<br />" .mysql_error();
    // puis redirige vers la page précédente
    echo "<script type='text/javascript'>document.location='" .$referer ."';</script>";
    exit();
  }
  ...
  // ajoute le message à la session
  $_SESSION['success'] = 1;
  $_SESSION['msg'] = _("L'attribut a été ajouté avec succès");
  // puis redirige vers la page précédente
