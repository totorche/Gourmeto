<head>
  <?php
    ...
    try{
      ...
      $promotions = Miki_shop_promotion::get_all_promotion($article->id);
    }
    catch(Exception $e){
      echo "<script type='text/javascript'>document.location='" .$_SESSION['url_back'] ."';</script>";
    }
...
function action_send(){
  ...
  if ($('action').value == "delete"){
    if (confirm("<?php echo _('Etes-vous sûr de vouloir supprimer les éléments sélectionnés ?'); ?>"))
      document.location = 'article_promotion_delete.php?id=' + ids;
  }
}

</script>
<div id="arianne">
  <a href="#"><?php echo _("Shop"); ?></a> > <a href="index.php?pid=143"><?php echo _("Liste articles"); ?></a> > Promotions
</div>
...
<div id="first_contener">
  <h1><?php echo _("Promotions de l'article '") .$article->get_name('fr') ."'"; ?></h1>  
  
  <div style="margin-bottom:10px">
    <?php 
    if (test_right(51)){ ?>
      <a href="index.php?pid=151&id=<?php echo $article->id; ?>" title="<?php echo _("Ajouter une promotion pour cet article"); ?>"><img src="pictures/newobject.gif" border="0" alt="<?php echo _("Ajouter une promotion pour cet article"); ?>" style="vertical-align:middle" /><?php echo _("Ajouter une promotion pour cet article"); ?></a>
    <?php } ?>
  </div>
  
  <table id="main_table">
    <tr class="headers">
      <td style="width:30%;color:#ffffff;font-weight:bold"><?php echo _("Date de début"); ?></td>
      <td style="width:30%;color:#ffffff;font-weight:bold"><?php echo _("Date de fin"); ?></td>
    ...
    <?php
      if (sizeof($promotions) == 0)
        echo "<tr><td colspan='4'>" ._("Aucune promotion n'est disponible pour cet article.") ."</td></tr>";
      
      $n = 0;
      //for ($x=0; $x < ($max_elements + $start) && $x < sizeof($articles); $x++){
      foreach($promotions as $promo){
        $date_start = explode(" ", $promo->date_start);
        $date_start = explode("-", $date_start[0]);
        $date_start = $date_start[2] .'/' .$date_start[1] .'/' .$date_start[0];
        
        $date_stop = explode(" ", $promo->date_stop);
        ...
        $n = ($n+1)%2;
        echo "
          <tr id='$promo->id' class='pages' onmouseover=\"colorLine('$promo->id');\" onmouseout=\"uncolorLine('$promo->id');\">
            ...
            <td class='$class' style='height:2em'>"
              .number_format($promo->price,2,'.',"'") ." CHF
            </td>
            <td class='$class' style='height:2em;height:20px;text-align:right;padding-right:10px'>";
              if (test_right(52))
                echo "<span style='margin-right:10px'><a href='index.php?pid=152&id=$promo->id' title='" ._('Editer') ."'><img src='pictures/edit.gif' border='0' alt='" ._('Editer') ."' /></a></span>";
              if (test_right(53))
                echo "<span style='margin-right:10px'><a class='delete' href='article_promotion_delete.php?id=$promo->id' title='" ._('Supprimer') ."'><img src='pictures/delete.gif' border='0' alt='" ._('Supprimer') ."' /></a></span>";
              
              echo "<span><input type='checkbox' class='check_element' element_id='$promo->id' /></span>
            </td>
          </tr>";
      }
  ?>
