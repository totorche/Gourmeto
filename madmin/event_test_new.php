<?php
    ...
    try{
      ...
      try{
        ...
        for($x=1; $x<=$nb_pictures; $x++){
          ...
          if ($image['error'] != 4){
            $event->upload_file($image, $titres_images, $lang_image);
          }
        }
        //$event->files_name = $titres_images;
        $event->update();
      }catch(Exception $e){
        $referer = "index.php?pid=192";
        // supprime l'événement
        $event->delete();
        // sauvegarde l'événement dans la session
        $_SESSION['saved_event'] = $event;
        $_SESSION['success'] = 0;
        $_SESSION['msg'] = "Une erreur est survenue lors de l'ajout de l'événement : <br />" .$e->getMessage();
  ...
  // ajoute le message à la session
  $_SESSION['success'] = 1;
  $_SESSION['msg'] = _("L'événement a été créé avec succès");
  // puis redirige vers la page précédente
